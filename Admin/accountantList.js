$(document).ready(function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      //alert(myhtml);
      $("#adminaccountantListHeader").empty();
      $("#adminaccountantListHeader").append(
        ' <div class="d-flex flex-column justify-content-center" style="min-height: 300px"><h3 class="display-4 text-white text-uppercase">Accountant List</h3><div class="d-inline-flex text-white"> <p class="m-0 text-uppercase"><a class="text-white" href="../Admin/home.html">Home</a></p><i class="fa fa-angle-double-right pt-1 px-3"></i><p class="m-0 text-uppercase">Admin</p></div></div>'
      );

      $("#adminaccountantList").empty();
      $("#adminaccountantList").append(myhtml);
    },
    error: function (data) {
      alert("Not Accountant Exists");
...
$(document).on("click", ".student_view", function (e) {
  ...
  var myrequest = $.ajax({
    url: "http://localhost:8080/accountants/" + userId, //'http://localhost:8082/applicants/'+userId,
    type: "GET",
    dataType: "json",
    async: false,
    processData: false,
  ...
  myrequest.done(function (response, textStatus, jqXHR) {
    ...
    $("#adminaccountantList").empty();
    $("#adminaccountantList").append(studentHtml);
    alert("empty");
  });
...
$(document).on("click", ".student_delete", function (e) {
  ...
  if (confirm("Are You sure ?")) {
    ...
    window.location.replace("http://127.0.0.1:5502/Admin/accountantList.html");
  } else {
...
function get_data_with_html_for_view_recruiter(response) {
  ...
  var post_html = `</tbody>
                    ...
                    <a class="btn btn-primary btn-sm" href="../Admin/accountantList.html">Back</a>`;
...
// console.log("Hello");
// const url = "http://localhost:8080/accountants";
// const accountantName = document.querySelector('#name');
// const gender = document.querySelector('#gender');
// const email= document.querySelector('#email');
// const mobileNo= document.querySelector('#mobileNo');
// // const yearOfPublication= document.querySelector('#yearOfPublication');
...
//     let data = xHR.response;
//     data.forEach(function(val){
//         accountantName.innerHTML += `${val.firstName} <br><br>` ;
//         gender.innerHTML +=`${val.lastName} <br><br>` ;
//         email.innerHTML +=`${val.email} <br><br>` ;
//         mobileNo.innerHTML +=`${val.dob} <br><br>`;
//         // yearOfPublication.innerHTML +=`${val.publication_date} <br><br>` ;
//         // discription.innerHTML +=`${val.description} <br><br>` ;
//         // price.innerHTML +=`${val.price} <br><br>` ;
//        edit.innerHTML += `  <a class="btn btn-primary btn-sm appliedJobs" id=""><i class="fas fa-pen"></i></a>`;
//        view.innerHTML += ` <a class="btn btn-primary btn-sm appliedJobs" id=""><i class="fas fa-eye"></i></a> <br><br>`;
//        del.innerHTML += ` <a class="btn btn-primary btn-sm appliedJobs" id=""><i class="fas fa-trash"></i></a> <br><br>`;
